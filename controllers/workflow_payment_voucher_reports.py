...
def get_account_voucher_canvas(canvas, doc):
    ...
    _top = [
        ['PAYMENT VOUCHER'],
        [str(_id.account_voucher_transaction_code)+str(_id.payment_voucher_no)],
        ['Pay To',':',_id.payee.upper(),'','Date',':',_id.payment_voucher_date.strftime("%d/%b/%Y")], #.strftime("%d/%b/%Y")
        ['Cheque No',':',_id.cheque_no,'','Cheque Dated',':',_id.cheque_dated.strftime("%d/%b/%Y")], #.strftime('%d/%b/%Y')
        ['Bank Name',':',str(_id.bank_name_id.bank_name) + ', ' + str(_id.bank_name_id.account_code),'','','',''],
    ...
    _bottom2 = [
        ['Payment Req.No.',':',_id.payment_voucher_request_no,'','Payment Req.Date',':',_id.transaction_reference_date.strftime("%d/%b/%Y")],
        ['Account Code',':',_id.account_code,'','Payment Mode',':',_id.account_payment_mode_id.account_voucher_payment_name],
        ['Remarks',':',_id.remarks,'','','','']]
    footer2 = Table(_bottom2, colWidths=[100,20,'*',30,100,20,'*'])
...
class PageNumCanvas(canvas.Canvas):
    ...
    def draw_page_number(self, page_count):
        ...
        printed_by = 'Printed By: '+ str(auth.user.first_name[0]) + '.' +str(auth.user.last_name)
        self.setFont("Courier", 7)
        # self.drawRightString(61*mm, 10*mm, PDFfile)
        self.drawRightString(200*mm, 10*mm, printed_on)
        self.drawRightString(200*mm, 7*mm, printed_by)
