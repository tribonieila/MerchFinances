...
def get_account_voucher_request_id():
    ...
    for n in db(db.Account_Voucher_Transaction_Request.account_voucher_request_id == request.args(0)).select():
        ...
        _row.append([
            ...
            locale.format('%.3F',n.amount_paid or 0, grouping = True),
            ])
        _total_amount += float(n.amount_paid or 0)

    (_whole, _frac) = (int(_total_amount), locale.format('%.3f',_total_amount or 0, grouping = True))
    _amount_in_words = 'QAR ' + string.upper(w.number_to_words(_whole, andword='')) + ' AND ' + str(str(_frac)[-2:]) + '/100 DIRHAMS'

    _row.append(['','','','','','Total Amount (QAR): ','',locale.format('%.3F', _total_amount or 0, grouping = True)])
